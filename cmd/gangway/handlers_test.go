		...
		t.Run(name, func(t *testing.T) {
			...
			if rsp.Code == 200 {
				...
				if authInfo.AuthInfo.Token != "" {
					t.Errorf("Expected Token field to be empty (using exec format), but got %q", authInfo.AuthInfo.Token)
				}
				if authInfo.AuthInfo.AuthProvider != nil {
					t.Errorf("AuthProvider should be nil (using exec format), got %v", authInfo.AuthInfo.AuthProvider)
				}
